...
const CLOUDINARY_FOLDER_TO_SCAN = 'event';
 ...
 * This looks for exact strings that start with 'event/' inside .ts, .tsx, .js, .json files.
 */
function scanCodeForPublicIds(): Set<string> {
  ...
  if (existsSync(productDataPath)) {
    try {
       const slugs = ['men', 'women', 'accessories', 'kids', 'jeans', 'lingerie', 'shoes'];
       slugs.forEach(slug => {
         validIds.add(`event/categories/${slug}/header`);
         for(let i=1; i<=6; i++) {
            validIds.add(`event/categories/${slug}/products/${i}`);
         }
       });
  ...
  // 3. Scan the codebase for raw "event/..." strings using a simple grep
  try {
    const grepCommand = `git grep -ho "event/[a-zA-Z0-9_/-]*" || echo ""`;
